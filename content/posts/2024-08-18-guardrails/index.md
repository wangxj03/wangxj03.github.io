...
output guardrails. Input guardrails filter out inappropriate data before it's
processed by the AI system, while output guardrails prevent the system from
generating harmful content.

We'll adapt an example from the OpenAI cookbook on [how to use
guardrails](https://cookbook.openai.com/examples/how_to_use_guardrails). We'll
...
1. **Run chat completion and moderation guardrail** (includes two subtasks
...
will be canceled if it's still running. This early cancellation prevents
...
with OpenAI's chat completion API. You can test the service with a simple curl
command
...
With the guardrails in place, it's essential to track the latency for
understanding the user experience. To fully make sense of the response, it's
also important to observe the intermediate results of the guardrails, error
messages, and the traces of the requests passing through various system
components.
...
straightforward. We just need to add the `@observe()` decorator to the guardrail
and the chat completion functions we want to monitor.
